...
var userSetupModel = module.exports = {
  ...
  validationData: async function (params) {
    try {

      let replacementsData = [];
      let query = 'SELECT COUNT(*) AS matchData FROM  ';
      if ('emailId' in params) {
        query += 'user_details WHERE email_id=?';
        replacementsData.push(params.emailId);
      }
      else if ('mobileNo' in params) {
        query += 'user_details WHERE mobile_no=?';
        replacementsData.push(params.mobileNo);
      } else if ('userId' in params) {
        query += 'user_masters WHERE user_name=?';
        replacementsData.push(params.userId);
      }
      ...
      logger.error(`${path}validationData()- ${err}`)
    }
  }
}